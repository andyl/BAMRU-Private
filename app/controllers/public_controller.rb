class PublicController < ApplicationController
  ...
  def generate_csv2(events, options = {})
    require 'csv'
    labels = %w(kind title leaders start_date      finish_date      start_time      finish_time      location lat lon description)
    fields = %w(typ  title leaders csv_start_date  csv_finish_date  csv_start_time  csv_finish_time  location lat lon description)
    CSV.generate(options) do |csv|
      csv << labels
      events.each do |event|
        csv << fields.map { |f| event.send(f.to_sym) }
